...
import { updateSessionTokens, markEntraVerified, markAafMfaVerified, setAafOriginalState, updateSessionNonce, updateSessionUserClaims, BridgeSession } from '../models/session';
...
export async function callbackAaf(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Fetch additional claims from AAF MFA userinfo endpoint and merge into session
    if (mfaResult.accessToken && bridgeSession.user_claims) {
      const aafMfaClaims = await getAafMfaUserInfo(mfaResult.accessToken);
      if (Object.keys(aafMfaClaims).length > 0) {
        const existingClaims = JSON.parse(bridgeSession.user_claims) as Record<string, unknown>;
      // Merge AAF MFA claims additively, with existing session claims taking precedence.
        const mergedClaims = { ...aafMfaClaims, ...existingClaims };
        updateSessionUserClaims(bridgeState, mergedClaims);
      }
    }
...
export async function userinfo(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Find the active session for this subject to return enriched claims
...
// ---------------------------------------------------------------------------
// /entra-login — removed; use the standard OIDC authorization code flow
// ---------------------------------------------------------------------------
